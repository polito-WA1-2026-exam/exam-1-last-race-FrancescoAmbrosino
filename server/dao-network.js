...
// tutti i segmenti (coppie di stazioni adiacenti) UNICI
export const getSegments = async () => {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const [a, b] = prev.stationId < cur.stationId ? [prev, cur] : [cur, prev];
    const key = `${a.stationId}-${b.stationId}`;
    if (seen.has(key)) continue;
    seen.add(key);
    segments.push({ aId: a.stationId, aName: a.stationName, bId: b.stationId, bName: b.stationName });
  }
  return segments;
};

// id delle stazioni di interscambio
export const getInterchangeIds = async () => {
  const rows = await db.all(Q_INTERCHANGES);
  return new Set(rows.map(r => r.stationId));
};

// grafo di adiacenza non orientato
export const getAdjacency = async () => {
